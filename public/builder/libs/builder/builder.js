...
Vvveb.Builder = {
    ...
    saveAjax: function (fileName, startTemplateUrl, callback, saveUrl) {
        ...
        // url: '/builder/save.php',//set your server side save script url
...
Vvveb.FileManager = {
    ...
    init: function (allowedComponents = {}) {
        ...
        $(this.tree).on("click", ".set-frontend", function (e) {
            let element = $(e.target).closest("li");
            Vvveb.FileManager.setAsFrontend(element, e);
            e.preventDefault();
            return false;
        });
    ...
    setAsFrontend: function (element, e) {
        let page = element[0].dataset;
        if (confirm(`Are you sure you want to set "${page.file}" as frontend?`)) {
            $.ajax({
                type: "POST",
                url: "/admin/template/set",
                headers: {
    ...
    loadPage: function (name, allowedComponents = false, disableCache = true, loadComponents = false) {
        let page = $("[data-page-file='" + this.pages[name].file + "']", this.tree);
